...
** ACE Navigation
   ...
   First, create a new keymap that I'll bind to C-x C-a, this is going to
   be where all the ace navigation commands will get added.
   #+BEGIN_SRC emacs-lisp
     (setq ctl-x-a-map (make-sparse-keymap))
     (define-key ctl-x-map "\C-a" ctl-x-a-map)
   #+END_SRC
   Now add the ace navigation commands into the keymap.
   #+BEGIN_SRC emacs-lisp
   ...
   this and train my fingers to use the "C-x C-a j" binding instead.
   #+BEGIN_SRC emacs-lisp
